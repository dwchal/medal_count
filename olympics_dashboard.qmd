...
fetch_json <- function(url) {
  ...
  tryCatch(jsonlite::fromJSON(payload, flatten = TRUE), error = function(e) NULL)
}

# Try multiple endpoints in order, returning first successful result
fetch_json_any <- function(urls) {
  for (url in urls) {
...
# Fallback: scrape medal table from CBS Sports when the official API is unavailable
fetch_medal_table_web <- function() {
  url <- "https://www.cbssports.com/olympics/news/2026-winter-olympics-medal-count/"
  response <- tryCatch(
    httr::GET(url, httr::timeout(15),
              httr::add_headers(
                `User-Agent` = paste0("Mozilla/5.0 (X11; Linux x86_64) ",
                                      "AppleWebKit/537.36 (KHTML, like Gecko) ",
                                      "Chrome/120.0.0.0 Safari/537.36")
              )),
    error = function(e) NULL
  )
  if (is.null(response) || httr::http_error(response)) return(NULL)
  html <- httr::content(response, as = "text", encoding = "UTF-8")

  # Extract all <tr> body rows from the medal table
  rows <- regmatches(html,
    gregexpr('<tr class="TableBase-bodyTr">[\\s\\S]*?</tr>', html, perl = TRUE))[[1]]
  if (length(rows) == 0) return(NULL)

  result <- lapply(seq_along(rows), function(i) {
    row <- rows[[i]]
    # Extract 3-letter NOC code
    noc_m <- regexec('name--mobile">\\s*\\n?\\s*([A-Z]+)\\s*\\n?\\s*</span>', row, perl = TRUE)
    noc_v <- regmatches(row, noc_m)[[1]]
    if (length(noc_v) < 2) return(NULL)
    noc <- trimws(noc_v[2])

    # Extract the four numeric <td> values: gold, silver, bronze, total
    td_hits <- regmatches(row,
      gregexpr('<td class="TableBase-bodyTd">(\\d+)</td>', row, perl = TRUE))[[1]]
    nums <- as.integer(gsub('.*>(\\d+)<.*', '\\1', td_hits))
    if (length(nums) < 4) return(NULL)

    list(rank = i, country = noc,
         gold = nums[1], silver = nums[2], bronze = nums[3], total = nums[4])
  })
  result <- Filter(Negate(is.null), result)
  if (length(result) == 0) return(NULL)

  tibble(
    rank    = as.integer(sapply(result, `[[`, "rank")),
    country = sapply(result, `[[`, "country"),
    gold    = as.integer(sapply(result, `[[`, "gold")),
    silver  = as.integer(sapply(result, `[[`, "silver")),
    bronze  = as.integer(sapply(result, `[[`, "bronze")),
    total   = as.integer(sapply(result, `[[`, "total"))
  ) %>%
    mutate(
      flag = case_when(
        country == "USA" ~ "🇺🇸",
...
extract_field <- function(df, candidates) {
...
# Try multiple known endpoint patterns for the 2026 Winter Olympics
olympics_api_base <- "https://www.olympics.com/OG2026/odata"
olympics_api_bases <- c(
  "https://www.olympics.com/OG2026/odata",
  "https://api.olympics.com/OG2026/odata",
  "https://sph-s-api.olympics.com/winter/OG2026/odata"
)
...
# Try multiple medal standings endpoints
medal_api <- fetch_json_any(paste0(olympics_api_bases, "/MedalStandings"))

parse_medal_api <- function(api_result) {
  if (is.null(api_result)) return(NULL)
  # Handle OData-style response with "value" key
  raw <- if ("value" %in% names(api_result)) as_tibble(api_result$value)
         else if (is.data.frame(api_result)) as_tibble(api_result)
         else NULL
  if (is.null(raw) || nrow(raw) == 0) return(NULL)
  country_field <- extract_field(raw, c("Country", "NOC", "Organisation", "Org", "country", "CountryCode"))
  rank_field    <- extract_field(raw, c("Rank", "rank", "Position", "Pos"))
  gold_field    <- extract_field(raw, c("Gold", "GoldMedals", "gold", "G"))
  silver_field  <- extract_field(raw, c("Silver", "SilverMedals", "silver", "S"))
  bronze_field  <- extract_field(raw, c("Bronze", "BronzeMedals", "bronze", "B"))
  total_field   <- extract_field(raw, c("Total", "TotalMedals", "total", "T"))
  if (is.null(country_field) || is.null(gold_field) || is.null(silver_field) || is.null(bronze_field)) {
    return(NULL)
  }
  tibble(
    rank   = if (is.null(rank_field)) NA_integer_ else as.integer(rank_field),
    country = as.character(country_field),
    gold   = as.integer(gold_field),
    silver = as.integer(silver_field),
    bronze = as.integer(bronze_field),
    total  = if (is.null(total_field)) as.integer(gold_field) + as.integer(silver_field) + as.integer(bronze_field)
             else as.integer(total_field)
  ) %>%
    mutate(
      flag = case_when(
        country %in% c("USA", "United States", "United States of America", "US") ~ "🇺🇸",
        country %in% c("Norway", "NOR") ~ "🇳🇴",
        country %in% c("Germany", "GER") ~ "🇩🇪",
        country %in% c("Canada", "CAN") ~ "🇨🇦",
        country %in% c("Netherlands", "NED") ~ "🇳🇱",
        country %in% c("Sweden", "SWE") ~ "🇸🇪",
        country %in% c("Austria", "AUT") ~ "🇦🇹",
        country %in% c("Switzerland", "SUI") ~ "🇨🇭",
        country %in% c("Italy", "ITA") ~ "🇮🇹",
        country %in% c("France", "FRA") ~ "🇫🇷",
        country %in% c("China", "CHN") ~ "🇨🇳",
        country %in% c("Japan", "JPN") ~ "🇯🇵",
        country %in% c("South Korea", "Korea", "KOR") ~ "🇰🇷",
        TRUE ~ ""
      )
    )
}

parsed <- parse_medal_api(medal_api)
# If the official API is unavailable, fall back to scraping CBS Sports
if (is.null(parsed)) {
  parsed <- fetch_medal_table_web()
}
if (!is.null(parsed)) {
  medal_standings_live <- parsed
}

usa_live <- medal_standings_live %>%
  filter(country %in% c("USA", "United States", "United States of America", "US")) %>%
  slice(1)

# Current USA medal count: use live API data when available, otherwise 0 (unknown)
if (nrow(usa_live) > 0) {
  usa_current_gold <- usa_live$gold
  usa_current_silver <- usa_live$silver
  usa_current_bronze <- usa_live$bronze
} else {
  # No live data available — do not simulate; show 0 during Olympics, NA otherwise
  usa_current_gold <- 0L
  usa_current_silver <- 0L
  usa_current_bronze <- 0L
}
usa_current_total <- usa_current_gold + usa_current_silver + usa_current_bronze
api_data_available <- nrow(medal_standings_live) > 0
...
events_by_sport <- tibble(
  ...
  events = c(11, 11, 4, 12, 3, 5, 14, 2, 4, 3, 10, 2, 5, 5, 11, 14),
  usa_strength = c("Medium", "Low", "High", "Low", "Medium", "High",
                   "High", "High", "Medium", "Low", "Medium", "High",
                   "Low", "Low", "High", "Medium")
)

# Fetch live event schedule from the Olympics API
event_schedule <- tibble(
  ...
  event = character(),
  medals = logical(),
  usa_contender = logical()
)

schedule_api <- fetch_json_any(paste0(olympics_api_bases, "/Schedule"))
if (!is.null(schedule_api)) {
  sched_raw <- if ("value" %in% names(schedule_api)) as_tibble(schedule_api$value)
               else if (is.data.frame(schedule_api)) as_tibble(schedule_api)
               else NULL
  if (!is.null(sched_raw) && nrow(sched_raw) > 0) {
    date_field  <- extract_field(sched_raw, c("Date", "StartDate", "EventDate", "date"))
    time_field  <- extract_field(sched_raw, c("Time", "StartTime", "time"))
    sport_field <- extract_field(sched_raw, c("Sport", "SportName", "Discipline", "sport"))
    event_field <- extract_field(sched_raw, c("Event", "EventName", "Name", "event"))
    medal_field <- extract_field(sched_raw, c("Medal", "IsMedal", "MedalEvent", "medals"))
    usa_field   <- extract_field(sched_raw, c("USAContender", "UsaEntry", "usa_contender"))
    if (!is.null(date_field) && !is.null(sport_field)) {
      event_schedule <- tibble(
        date          = as.Date(substr(as.character(date_field), 1, 10)),
        time          = if (is.null(time_field)) NA_character_ else as.character(time_field),
        sport         = as.character(sport_field),
        event         = if (is.null(event_field)) NA_character_ else as.character(event_field),
        medals        = if (is.null(medal_field)) NA else as.logical(medal_field),
        usa_contender = if (is.null(usa_field)) NA else as.logical(usa_field)
      ) %>% filter(!is.na(date))
    }
  }
}
...
# Medal standings: use live API data when available, otherwise empty (no fake simulation)
if (nrow(medal_standings_live) > 0) {
  medal_standings <- medal_standings_live %>%
    mutate(
      rank = ifelse(is.na(rank), row_number(), rank),
      total = ifelse(is.na(total), gold + silver + bronze, total)
    ) %>%
    arrange(rank, desc(total), desc(gold), desc(silver)) %>%
    select(rank, country, flag, gold, silver, bronze, total) %>%
    slice_head(n = 10)
} else {
  # No live data available — show empty standings with a note
...
# Medals awarded: derive from live standings when available
if (nrow(medal_standings_live) > 0) {
  medals_awarded_gold   <- sum(medal_standings_live$gold,   na.rm = TRUE)
  medals_awarded_silver <- sum(medal_standings_live$silver, na.rm = TRUE)
  medals_awarded_bronze <- sum(medal_standings_live$bronze, na.rm = TRUE)
} else {
  medals_awarded_gold   <- 0L
  medals_awarded_silver <- 0L
  medals_awarded_bronze <- 0L
}
...
# Calculate if USA is on pace (during Olympics) — only meaningful with real data
on_pace_status <- if (!api_data_available) {
  "Live data unavailable"
} else if (olympics_started & !olympics_ended) {
...
winter_records <- tibble(
  ...
  Holder = c(
    "Norway (Beijing 2022)",
    "Norway (PyeongChang 2018)",
    "Norway",
    "Norway",
    "Tara Lipinski (USA, 1998)",
...
usa_milestones <- tibble(
  ...
  Detail = c(
    ...
    "10 golds (Salt Lake City 2002, home advantage)",
    "37 medals (Vancouver 2010)",
    "2nd overall (multiple Games)",
    "Apolo Ohno - 8 medals (2G, 2S, 4B) in Short Track",
...
# ============================================
# USA INDIVIDUAL MEDAL RESULTS
# ============================================

# Fetch USA medal results from the live Olympics API
usa_medal_results <- tibble(
  date    = as.Date(character()),
  sport   = character(),
  event   = character(),
  athlete = character(),
...
results_api <- fetch_json_any(c(
  paste0(olympics_api_bases, "/Results?$filter=NOC eq 'USA'"),
  paste0(olympics_api_bases, "/Medallists?$filter=NOC eq 'USA'"),
  paste0(olympics_api_bases, "/MedalResults?$filter=Country eq 'USA'")
))

if (!is.null(results_api)) {
  res_raw <- if ("value" %in% names(results_api)) as_tibble(results_api$value)
             else if (is.data.frame(results_api)) as_tibble(results_api)
             else NULL
  if (!is.null(res_raw) && nrow(res_raw) > 0) {
    date_field    <- extract_field(res_raw, c("Date", "EventDate", "date", "CompetitionDate"))
    sport_field   <- extract_field(res_raw, c("Sport", "Discipline", "sport"))
    event_field   <- extract_field(res_raw, c("Event", "EventName", "Name", "event"))
    athlete_field <- extract_field(res_raw, c("Athlete", "AthleteFullName", "Name", "athlete", "Competitor"))
    medal_field   <- extract_field(res_raw, c("Medal", "MedalType", "medal", "MedalCode"))
    if (!is.null(date_field) && !is.null(medal_field)) {
      usa_medal_results <- tibble(
        date    = as.Date(substr(as.character(date_field), 1, 10)),
        sport   = if (is.null(sport_field)) NA_character_ else as.character(sport_field),
        event   = if (is.null(event_field)) NA_character_ else as.character(event_field),
        athlete = if (is.null(athlete_field)) NA_character_ else as.character(athlete_field),
        medal   = as.character(medal_field)
      ) %>%
        filter(!is.na(date), !is.na(medal)) %>%
        filter(medal %in% c("Gold", "Silver", "Bronze", "GOLD", "SILVER", "BRONZE")) %>%
...
# Show USA rank from live standings; "--" when data is unavailable
usa_rank_row <- medal_standings %>% filter(country == "USA")
rank_display <- if (nrow(usa_rank_row) > 0) {
  paste("#", usa_rank_row$rank[1])
} else if (olympics_started) {
  "--"
} else {
  "--"
...
if (nrow(medal_standings) > 0) {
  ...
  cat("Live medal standings are not yet available. Check back when the Olympics API is accessible.")
}
```
...
if (nrow(upcoming_events) > 0) {
  upcoming_events %>%
    mutate(
      ...
      Time = time,
      `Medal Event` = ifelse(!is.na(medals) & medals, "🏅 Yes", ""),
      `USA Contender` = ifelse(!is.na(usa_contender) & usa_contender, "🇺🇸 Yes", "")
    ) %>%
  ...
  cat("Live event schedule is not available. Schedule data will appear here when the Olympics API is accessible.")
}
```
...
```{r}
#| content: valuebox
#| title: "USA Medals Won"
#| icon: trophy-fill
#| color: info
list(
  value = nrow(usa_medal_results)
)
```
...
#| color: warning
list(
  value = sum(usa_medal_results$medal == "Gold")
)
```
...
#| color: light
list(
  value = sum(usa_medal_results$medal == "Silver")
)
```
...
#| color: danger
list(
  value = sum(usa_medal_results$medal == "Bronze")
)
```
...
```{r}
#| title: "Every USA Medal — Milano Cortina 2026"

if (nrow(usa_medal_results) > 0) {
  usa_medal_results %>%
    mutate(
      Date = format(date, "%a, %b %d"),
      Medal = case_when(
        medal == "Gold"   ~ "\U0001F947 Gold",
        medal == "Silver" ~ "\U0001F948 Silver",
        medal == "Bronze" ~ "\U0001F949 Bronze"
      )
    ) %>%
    select(Date, Sport = sport, Event = event, Athlete = athlete, Medal) %>%
    gt() %>%
    cols_align(align = "center", columns = c(Date, Medal)) %>%
    cols_align(align = "left", columns = c(Sport, Event, Athlete)) %>%
    tab_style(
      style = cell_fill(color = "#fff9e6"),
      locations = cells_body(rows = Medal == "\U0001F947 Gold")
    ) %>%
    tab_style(
      style = cell_fill(color = "#f5f5f5"),
      locations = cells_body(rows = Medal == "\U0001F948 Silver")
    ) %>%
    tab_style(
      style = cell_fill(color = "#fdf0e2"),
      locations = cells_body(rows = Medal == "\U0001F949 Bronze")
    ) %>%
    tab_style(
      style = cell_text(weight = "bold"),
      locations = cells_body(columns = Medal)
    ) %>%
    cols_width(
      Date ~ px(120),
      Sport ~ px(140),
  ...
  cat("The Olympics have not started yet. Medal results will appear here once competition begins.")
} else {
  cat("Live medal results are not yet available. Data will populate here as the Olympics API becomes accessible.")
}
```
...
if (nrow(usa_medal_results) > 0) {
  sport_summary <- usa_medal_results %>%
    count(sport, medal) %>%
    mutate(medal = factor(medal, levels = c("Gold", "Silver", "Bronze"))) %>%
    arrange(sport, medal)

  sport_totals <- usa_medal_results %>%
    count(sport, name = "total") %>%
    arrange(desc(total))

  plot_ly(data = sport_summary,
          y = ~factor(sport, levels = rev(sport_totals$sport)),
          x = ~n,
          color = ~medal,
          colors = c("Gold" = "#FFD700", "Silver" = "#C0C0C0", "Bronze" = "#CD7F32"),
          type = "bar",
    ...
    layout(
      title = list(text = ""),
      xaxis = list(title = "Medals", dtick = 1,
                   gridcolor = "rgba(128,128,128,0.2)"),
      yaxis = list(title = "", gridcolor = "rgba(128,128,128,0.2)"),
      barmode = "stack",
      legend = list(orientation = "h", y = -0.15),
      margin = list(l = 130, t = 20),
      paper_bgcolor = "rgba(0,0,0,0)",
      plot_bgcolor = "rgba(0,0,0,0)"
    )
} else {
  plot_ly() %>%
    layout(
      title = list(text = "Awaiting results"),
      paper_bgcolor = "rgba(0,0,0,0)",
      plot_bgcolor = "rgba(0,0,0,0)"
    )
}
...
```{r}
#| title: "Medals by Date"
#| padding: 0

if (nrow(usa_medal_results) > 0) {
  daily_summary <- usa_medal_results %>%
    mutate(date_label = format(date, "%b %d")) %>%
    count(date_label, date, medal) %>%
    mutate(medal = factor(medal, levels = c("Gold", "Silver", "Bronze"))) %>%
    arrange(date)

  plot_ly(data = daily_summary,
          x = ~date_label,
          y = ~n,
          color = ~medal,
          colors = c("Gold" = "#FFD700", "Silver" = "#C0C0C0", "Bronze" = "#CD7F32"),
    ...
    layout(
      title = list(text = ""),
      xaxis = list(title = "", categoryorder = "array",
                   categoryarray = format(sort(unique(daily_summary$date)), "%b %d"),
                   gridcolor = "rgba(128,128,128,0.2)"),
      yaxis = list(title = "Medals", dtick = 1,
                   gridcolor = "rgba(128,128,128,0.2)"),
      barmode = "stack",
      legend = list(orientation = "h", y = -0.15),
      margin = list(t = 20),
...
```{r}
#| title: "Global Medal Distribution"
#| padding: 0

# Mapping from NOC codes and country names to ISO-3 codes for the choropleth map
country_iso_map <- tibble(
  noc       = c("USA", "NOR", "GER", "CAN", "NED", "SWE", "AUT", "SUI",
                "ITA", "FRA", "CHN", "JPN", "KOR"),
  iso3      = c("USA", "NOR", "DEU", "CAN", "NLD", "SWE", "AUT", "CHE",
                "ITA", "FRA", "CHN", "JPN", "KOR"),
  name_long = c("USA", "Norway", "Germany", "Canada", "Netherlands", "Sweden",
                "Austria", "Switzerland", "Italy", "France", "China", "Japan",
                "South Korea")
)

if (api_data_available && nrow(medal_standings_live) > 0) {
  medal_map_data <- medal_standings_live %>%
    inner_join(country_iso_map %>% select(noc, iso3, name_long),
               by = c("country" = "noc")) %>%
    select(country = name_long, iso3, gold, silver, bronze, total, flag)
  map_source_label <- "Live Standings"
} else {
  medal_map_data <- country_predictions %>%
    inner_join(country_iso_map %>% select(name_long, iso3),
               by = c("country" = "name_long")) %>%
    select(country, iso3, gold = pred_gold, silver = pred_silver,
           bronze = pred_bronze, total = pred_total, flag)
  map_source_label <- "2026 Projections"
}

medal_map_data <- medal_map_data %>%
  mutate(
    hover_text = paste0(
      "<b>", flag, " ", country, "</b><br>",
      "\U0001F947 Gold: ", gold, "<br>",
      "\U0001F948 Silver: ", silver, "<br>",
      "\U0001F949 Bronze: ", bronze, "<br>",
      "<b>Total: ", total, "</b>"
    )
  )

max_medals <- max(medal_map_data$total, na.rm = TRUE)

plot_geo(medal_map_data, locations = ~iso3, locationmode = "ISO-3") %>%
  add_trace(
    z          = ~total,
    text       = ~hover_text,
    hoverinfo  = "text",
    colorscale = list(
      c(0,   "rgba(220,230,245,0.4)"),
      c(0.2, "#7aacd6"),
      c(0.5, "#3C7FC4"),
      c(0.8, "#1a4a8e"),
      c(1,   "#0a1f50")
    ...
    colorbar = list(
      title     = list(text = "Total<br>Medals"),
      thickness = 14,
      len       = 0.55,
      x         = 1.01
    ),
    marker = list(line = list(color = "rgba(200,200,200,0.4)", width = 0.5)),
    zmin = 0,
    zmax = max_medals
  ) %>%
  layout(
    title = list(
      text = paste0("2026 Winter Olympics \u2014 ", map_source_label),
      font = list(size = 13)
    ),
    geo = list(
      showframe      = FALSE,
      showcoastlines = TRUE,
      coastlinecolor = "rgba(160,160,160,0.4)",
      showland       = TRUE,
      landcolor      = "rgba(230,230,230,0.25)",
      showocean      = TRUE,
      oceancolor     = "rgba(200,220,240,0.18)",
      showlakes      = FALSE,
      showrivers     = FALSE,
...
plot_ly(
  data        = medal_map_long,
  y           = ~reorder(paste(flag, country), total),
  x           = ~count,
  color       = ~medal_type,
  colors      = c("Gold" = "#FFD700", "Silver" = "#C0C0C0", "Bronze" = "#CD7F32"),
  type        = "bar",
  orientation = "h",
  text        = ~ifelse(count > 0, as.character(count), ""),
  textposition     = "inside",
  insidetextanchor = "middle"
) %>%
  layout(
    barmode = "stack",
    xaxis   = list(title = "Medals", gridcolor = "rgba(128,128,128,0.2)"),
    yaxis   = list(title = ""),
    legend  = list(orientation = "h", y = 1.08, x = 0.5,
                   xanchor = "center", traceorder = "reversed"),
    margin  = list(l = 130, r = 10, t = 40, b = 40),
    paper_bgcolor = "rgba(0,0,0,0)",
    plot_bgcolor  = "rgba(0,0,0,0)"
  )
```
...
```{r}
#| title: "Top Medal Countries"

medal_map_data %>%
  arrange(desc(gold), desc(total)) %>%
  head(8) %>%
  mutate(Country = paste(flag, country)) %>%
  select(Country, Gold = gold, Silver = silver, Bronze = bronze, Total = total) %>%
  gt() %>%
  cols_align(align = "center", columns = c(Gold, Silver, Bronze, Total)) %>%
  cols_align(align = "left",   columns = Country) %>%
  tab_style(
    style     = list(cell_fill(color = "#FFD70022"), cell_text(weight = "bold")),
    locations = cells_body(rows = 1)
  ) %>%
  data_color(
    columns = Total,
    palette = c("#f0f4ff", "#3C3B6E")
  ) %>%
  tab_style(
    style     = cell_text(color = "#B8860B", weight = "bold"),
...
cat("- **World medal map** showing global medal distribution via interactive choropleth\n\n")
...
cat("### Data Sources\n\n")
cat("- Historical medal data (2002-2022 Winter Olympics) is entered manually for demo purposes\n")
cat("- Medal standings attempt to load from the olympics.com live API when available\n")
cat("- Event schedule and sport strength ratings are simulated placeholders until official feeds are published\n\n")
